#!/usr/bin/env python
"""
Sample Data Seeding Script for Cosmetic Manufacturing System
...
from CustomerAndSalesManagement.models import *
from SupplierManagement.models import *
from WarehouseManagement.models import *
from EmployeeManagement.models import *
from CostAnalysisManagement.models import *
...
def create_sample_equipment():
    ...
    equipment_data = [
        {'id': 'EQ001', 'category': 'Test Tube', 'condition': 'Brand New'},
        {'id': 'EQ002', 'category': 'Flask', 'condition': 'Used'},
        {'id': 'EQ003', 'category': 'Beaker', 'condition': 'Brand New'},
        {'id': 'EQ004', 'category': 'Pipette', 'condition': 'Used'},
        {'id': 'EQ005', 'category': 'Burette', 'condition': 'Need Repair'},
        {'id': 'EQ006', 'category': 'Measuring Cylinder', 'condition': 'Brand New'},
        {'id': 'EQ007', 'category': 'Laboratory Stand', 'condition': 'Used'},
        {'id': 'EQ008', 'category': 'Funnel', 'condition': 'Brand New'}
    ]
...
def create_sample_chemicals():
    ...
    chemicals_data = [
        {'name': 'Sodium Hydroxide', 'quantity': 500.0, 'status': 'Available'},
        {'name': 'Hydrochloric Acid', 'quantity': 300.0, 'status': 'Available'},
        {'name': 'Ethanol', 'quantity': 1000.0, 'status': 'Available'},
        {'name': 'Distilled Water', 'quantity': 5000.0, 'status': 'Available'},
        {'name': 'Phenolphthalein', 'quantity': 50.0, 'status': 'Available'},
        {'name': 'Methyl Orange', 'quantity': 25.0, 'status': 'Not Available'}
    ]
...
def create_sample_batches(productions):
    """Create sample batches"""
    print("\nCreating sample batches...")
    
    for production in productions_data:
        batch, created = Batch.objects.get_or_create(
            schedule_id=production,
            defaults={
                'batch_quantity': production.target_quantity,
        ...
        if created:
            print(f"✓ Created batch for: {production.product_code.product_name}")
        else:
            print(f"✓ Batch already exists for: {production.product_code.product_name}")
    ...
    machinery_data = [
        {
            ...
            'date_purchased': datetime.now().date() - timedelta(days=365),
            'repair_duration': 30,
            'condition': 'Brand New'
        },
        {
            ...
            'date_purchased': datetime.now().date() - timedelta(days=730),
            'repair_duration': 45,
            'condition': 'Used'
        },
        {
            ...
            'date_purchased': datetime.now().date() - timedelta(days=180),
            'repair_duration': 30,
            'condition': 'Brand New'
        }
    ]
...
def main():
    ...
    try:
        ...
        # Get productions for batches
        productions = ScheduleProduction.objects.all()
        create_sample_batches(productions)
